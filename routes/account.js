...
/**
 * @api {put} /api/accounts/orders Update orders
 ...
 *     }
 */
router.route('/orders').put(checkJWT, async (req, res) => {
    let data = await dbAccount.updateOrders(req.body.orders, req.decoded.user._id);
    return res.status(data['meta'].code).send(data);
});
